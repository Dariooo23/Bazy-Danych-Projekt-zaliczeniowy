...
class MySQLGui:
    ...
    def delete_row(self):
        ...
        # pobierz opis tabeli by znaleźć kolumny i klucz(e) główne
        self.cursor.execute(f"DESCRIBE `{table}`")
        descs = self.cursor.fetchall()
        columns = [c[0] for c in descs]

        # znajdź wszystkie kolumny PK (może być więcej niż jedna)
        ...
        if pk_cols:
            ...
            # brak PK -> fallback: użyjemy wszystkich kolumn by zidentyfikować wiersz
            where_cols = columns
            where_vals = list(values)

        # używamy operatora <=> (NULL-safe) żeby poprawnie porównywać NULL'e
    ...
    def _handle_db_error(self, err):
        # rollback any partial transaction and show a user-friendly error
        try:
            self.conn.rollback()
        except Exception:
            pass
        ...
        # MySQL uses SQLSTATE '23000' for integrity constraint violations; also check message for 'check'
    ...
    def open_editor(self, title, values=None):
        table = self.table_name.get()
        # pobierz opis tabeli (DESCRIBE zwraca: Field, Type, Null, Key, Default, Extra)
        self.cursor.execute(f"DESCRIBE `{table}`")
        descs = self.cursor.fetchall()
        columns = [c[0] for c in descs]

        # mapy pomocnicze o kolumnach
        null_allowed = {c[0]: (len(c) >= 3 and c[2] == 'YES') for c in descs}

        # znajdź klucz główny (jeśli jest)
        pk = None
        pk_extra = ''
        for d in descs:
            if len(d) >= 4 and d[3] == 'PRI':
                ...
                break
        # UWAGA: nie ustawiamy pk na pierwszą kolumnę, jeśli brak klucza głównego
        ...
        # jeśli podano wartości, zmapuj je na kolumny (bez założenia o porządku)
        ...
        for i, col in enumerate(columns):
            ...
            if values:
                v = val_map.get(col, '')
                # pokaż NULL jako pusty string w UI
                e.insert(0, '' if v is None else v)
            entries[col] = e

        def _to_param(col, raw_val):
            # konwertuj puste stringi na None jeśli kolumna dopuszcza NULL
        ...
        def save():
            try:
                ...
                if values:
                    # jeśli mamy PK, nie aktualizujemy jej i używamy PK w WHERE
                    if pk:
                        ...
                        # brak PK: użyjemy WHERE z porównaniem wszystkich kolumn na podstawie oryginalnych wartości
                        # używamy operatora <=> by bezpiecznie porównywać NULL
                        set_cols = columns[:]
                        if not set_cols:
                            messagebox.showwarning("Błąd", "Brak kolumn do aktualizacji")
                            return
                        ...
                        # params: nowe wartości dla set_cols, a potem oryginalne wartości do WHERE
                        params = [_to_param(c, entries[c].get()) for c in set_cols] + list(values)
                        print(query, params)
                        self.cursor.execute(query, params)
                else:
                    # INSERT: pomiń auto_increment PK, jeśli występuje
                    insert_cols = columns[:]
